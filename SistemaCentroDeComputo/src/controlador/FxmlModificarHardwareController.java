...
public class FxmlModificarHardwareController implements Initializable {
  ...
  public void initialize(URL url, ResourceBundle rb) {
    ...
    comboBoxEstado
        .setOnAction(e -> System.out.println("selecciona un estado: " + comboBoxEstado.getValue()));

    buttonGuardar.setOnAction((ActionEvent event) -> {
      Validador validador = new Validador();

      boolean noInventarioValidado =
          validador.validarNoInventarioHardware(textFieldNoInventario.getText());
      ...
      if (noInventarioValidado == true && fechaValida == true && numSerie == true) {
        ...
        boolean resultadoActualizacion = hardwareDao.actualizarHardware(new Hardware(noInventarioUv,
            marca, modelo, numerodeSerie, estado, tipoDispositivo, fecha, responsable, ubicacion));

        if (resultadoActualizacion == true) {
          Alert alerta = new Alert(Alert.AlertType.INFORMATION);
          alerta.setTitle("Hardware Guardado");
          alerta.setHeaderText(null);
          alerta.setContentText("Los datos han sido guardados! Recuerda" + "actualizar la tabla!!");
          alerta.show();
        } else {
